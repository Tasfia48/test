...
var demo_dictionary = {};
 ...
 function update_demo_list(dist){
     // $("#method_list").empty()
     ...
     $.ajax({
         ...
         success: function(res) {
             if(res.msg != "success") alert(res.msg)
                 ...
                 console.log(res)
                 demo_dictionary=res.data;
                 ['WHITE','HISP','BLACK','ASIAN','OTHER'].forEach((item,i) => {
                   $("#text_"+item).html(((demo_dictionary[dist][item]/demo_dictionary[dist]['TOT_POP']*100).toFixed(2)))
                 });
                 $("#text_TOT_POP").html(demo_dictionary[dist]['TOT_POP'])

                 // console.log(demo_dictionary)
             }
         },
         error: function(error) {
             console.log(error);
...
*/
var demo_beat_dictionary = {};

 function update_beat_demo_list(beat1,beat2){
     // $("#method_list").empty()

     $.ajax({
         url: '/get_beat_demographics',
         type: 'POST',
         success: function(res) {
             if(res.msg != "success") alert(res.msg)
             else{
                 console.log(res)
                 demo_beat_dictionary=res.data;
                 ['WHITE','HISP','BLACK','ASIAN','OTHER'].forEach((item,i) => {
                   $("#text_"+item+"_one").html(((demo_beat_dictionary[beat1][item]/demo_beat_dictionary[beat1]['TOT_POP']*100).toFixed(2)))
                 });
                 ['WHITE','HISP','BLACK','ASIAN','OTHER'].forEach((item,i) => {
                   $("#text_"+item+"_two").html(((demo_beat_dictionary[beat2][item]/demo_beat_dictionary[beat2]['TOT_POP'])*100).toFixed(2))
                 });
                 $("#beat1").html(beat1)
                 $("#beat2").html(beat2)
                 $("#text_TOT_POP_one").html(demo_beat_dictionary[beat1]['TOT_POP'])
                 $("#text_TOT_POP_two").html(demo_beat_dictionary[beat2]['TOT_POP'])
    ...
    $.ajax({
        ...
        success: function(res) {
            if(res.msg != "success") alert(res.msg)
                ...
                //   $('#main_div').attr('<div class="col-lg-4"><image src='+img+'></div>');
                // })
...
function show_single_plots(highlight=false){
    $.ajax({
...
$("#update_button").on("click",function(){
  update()
  update_beat_demo_list($("#b1").val(),$("#b2").val())
  showDiv('demoBeatInfo')

})

$("#showButton").on("click",function(){show_single_plots()})
